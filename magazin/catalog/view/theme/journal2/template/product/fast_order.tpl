		...
		$('#content').on('click', '#add-product', function(){
			if ($('#fake-form')[0].checkValidity()) {
				...
				if ($('input[name="package-size"]').val() != 0 && $('input[name="package-discount"]') != 0) {
					product_row += appendCurrency(($('input[name="quantity"]').val() % $('input[name="package-size"]').val() * $('input[name="product-vat-price"]').val() + ($('input[name="quantity"]').val() - $('input[name="quantity"]').val() % $('input[name="package-size"]').val()) * $('input[name="product-vat-price"]').val() * (100 - $('input[name="package-discount"]').val()) / 100).toFixed(2)) + ' (<?php echo $text_without_vat; ?> ' + appendCurrency(($('input[name="quantity"]').val() % $('input[name="package-size"]').val() * $('input[name="product-base-price"]').val() + ($('input[name="quantity"]').val() - $('input[name="quantity"]').val() % $('input[name="package-size"]').val()) * $('input[name="product-base-price"]').val() * (100 - $('input[name="package-discount"]').val()) / 100).toFixed(2)) + ')';
				} else {
					product_row += appendCurrency($('input[name="quantity"]').val() * $('input[name="product-vat-price"]').val()) + ' (<?php echo $text_without_vat; ?> ' + appendCurrency($('input[name="quantity"]').val() * $('input[name="product-base-price"]').val()) + ')';
				}
				product_row += '</td>';
