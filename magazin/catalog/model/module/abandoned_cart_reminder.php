...
class ModelModuleAbandonedCartReminder extends Model {
	...
	public function getCustomersForReminder($data = array()){
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "customer 
				...
				WHERE TIMESTAMPDIFF(DAY, date_last_action, NOW()) >= " . (int)$this->config->get('abandoned_cart_reminder_delay') . "

					  AND TIMESTAMPDIFF(DAY, date_last_action, NOW()) <= " . (int)$this->config->get('abandoned_cart_max_reminder_delay') . "
